...
if ($_POST['ime_rang_prof'])
{
	$ime_prof = $_POST['ime_rang_prof'];
}
else
{
	echo "ne go zemal";
...
$id_prof = '';
$get_id = mysqli_query($connection, "SELECT id_profesor FROM profesori WHERE ime_prezime_prof = '$ime_prof'");
while ($get = $get_id -> fetch_assoc()){
	$id_prof = $get['id_profesor'];
}
...
$get_all3 = mysqli_query($connection, "SELECT * FROM prof_rang WHERE id_prof=$id_prof ");
...
$input_prosek_predavanja=" UPDATE profesori SET prosek_predavanja=(SELECT AVG (predavanja) FROM prof_rang) WHERE id_profesor=$id_prof ";
  mysqli_query($connection,$input_prosek_predavanja);
  $input_prosek_literatura=" UPDATE profesori SET prosek_literatura=(SELECT AVG (literatura) FROM prof_rang) WHERE id_profesor=$id_prof ";
